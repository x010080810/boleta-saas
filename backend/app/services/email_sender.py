...
def _send_via_smtp(
    ...
    try:
        if pdf_bytes:
            ...
            msg["Subject"] = subject
            _build_headers(msg, from_email, from_name, to_email)
            _attach_html_alternative(msg, html_body)
...
def _dispatch_email(
    ...
    if smtp_host and smtp_user and smtp_password:
        methods.append(("SMTP", lambda: _send_via_smtp(smtp_host, smtp_port, smtp_user, smtp_password, to_email, subject, html_body, from_email, from_name, pdf_bytes, pdf_filename)))
...
def send_payslip_email(
    ...
    return _dispatch_email(
        ...
        from_email=from_email, from_name=from_name or company_name,
        pdf_bytes=pdf_bytes, pdf_filename=pdf_filename,
...
def send_notification(
    ...
    return _dispatch_email(
        ...
        html_body=body,
        from_email=from_email,
        from_name=from_name or empresa,
        smtp_host=smtp_host, smtp_port=smtp_port,
        smtp_user=smtp_user, smtp_password=smtp_password,
    )
...
def send_welcome_email(
    ...
    return _dispatch_email(
        ...
        smtp_host=system_smtp_host, smtp_port=system_smtp_port,
        smtp_user=system_smtp_user, smtp_password=system_smtp_password,
    )
...
def send_license_expiry_warning(
    ...
    return _dispatch_email(
        ...
        html_body=body,
        from_email=from_email,
        from_name=from_name or "Sistema de Boletas",
        smtp_host=smtp_host, smtp_port=smtp_port,
        smtp_user=smtp_user, smtp_password=smtp_password,
    )
...
def send_user_welcome_email(
    ...
    <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: white; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
            ...
            <p>Su cuenta en <strong>Boleta SaaS</strong> ha sido creada para la empresa <strong>{company_name}</strong>.</p>
            <div style="background: #f0fdf4; border: 1px solid #22c55e; padding: 20px; border-radius: 8px; margin: 20px 0;">
                <h3 style="color: #166534; margin: 0 0 10px;">Datos de Acceso</h3>
                <p style="margin: 5px 0;"><strong>Usuario:</strong> {to_email}</p>
                <p style="margin: 5px 0;"><strong>Contraseña:</strong> {password}</p>
            </div>
            <p>Puede ingresar al sistema con su correo electronico y la contraseña proporcionada.</p>
            <p style="color: #6b7280; font-size: 12px; margin-top: 30px;">Saludos cordiales,<br><strong>Boleta SaaS</strong></p>
        </div>
    </div>
    </body></html>
    """
...
def send_new_user_notification(
    ...
    <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background: linear-gradient(135deg, #6366f1, #4f46e5); padding: 25px; border-radius: 10px 10px 0 0; text-align: center;">
            <h2 style="color: white; margin: 0;">Nuevo Usuario Registrado</h2>
        </div>
        <div style="background: white; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
            <p>Se ha creado un nuevo usuario en la empresa <strong>{company_name}</strong>:</p>
            <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
                <tr><td style="padding: 8px; background: #f8f9fa;"><strong>Nombre</strong></td><td style="padding: 8px;">{new_user_name}</td></tr>
                <tr><td style="padding: 8px; background: #f8f9fa;"><strong>Email</strong></td><td style="padding: 8px;">{new_user_email}</td></tr>
                <tr><td style="padding: 8px; background: #f8f9fa;"><strong>Rol</strong></td><td style="padding: 8px;">{new_user_role}</td></tr>
            </table>
        </div>
    </div>
    </body></html>
    ...
    return _dispatch_email(
        to_email=to_email,
        subject=f"Nuevo usuario en {company_name}",
        html_body=body,
        from_email=system_from_email,
        from_name=system_from_name,
        smtp_host=system_smtp_host, smtp_port=system_smtp_port,
